  ...
  return (
    ...
    >
        ...
        <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
            {
                chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
                    ...
                    >
                        ...
                        <img src={assets.bin_icon} alt="bin_icon" className='hidden group-hover:block w-4 cursor-pointer not-dark:invert'/>
                    </div>
                ))
            }
        </div>
        ...
        <div onClick={() => {navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
            <img src={assets.diamond_icon} alt="diamond_icon" className='w-4.5 dark:invert'/>
            <div className='flex flex-col text-sm'>
        ...
        <div onClick={() => {navigate('/community'); scrollTo(0,0)}} className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
            <img src={assets.user_icon} alt="user_icon" className='w-7 rounded-full'/>
            <p className='flex-1 text-sm dark:text-primary truncate'>
                {user ? user.name : "Login your account"}
            </p>
            {user && <img src={assets.logout_icon} alt="logout_icon" className='h-5 cursor-pointer hidden not-dark:invert group-hover:block'/> }
        </div>
